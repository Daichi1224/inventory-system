...
export default function StockOutPage() {
  ...
  useEffect(() => {
    async function fetchItems() {
      const { data, error } = await supabase
        ...
        .select("id, item_code, item_name, current_stock")
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!quantity.trim()) {
      errors.quantity = "出庫数量を入力してください";
    } else if (!Number.isInteger(qty) || qty <= 0) {
      errors.quantity = "出庫数量は1以上の整数で入力してください";
    }

    if (!movementDate) {
      errors.movement_date = "出庫日を入力してください";
    }
    ...
    if (!selectedItem) {
      setSubmitError("選択された品目が見つかりません。");
      return;
    }

    if (qty > selectedItem.current_stock) {
      setFieldErrors({
        quantity: `在庫数を超える出庫はできません（現在在庫: ${selectedItem.current_stock}）`,
      });
      return;
    }
  ...
  return (
    <main className="page-container">
      <h1 className="page-title">出庫登録</h1>
      ...
      <div className="card form-card">
        <form onSubmit={handleSubmit} className="form-grid">
          ...
          {selectedItem ? (
            <p className="info-text">現在在庫: {selectedItem.current_stock}</p>
          ) : null}

          <div className="form-group">
            <label className="label">出庫数量</label>
            <input
              type="number"
              value={quantity}
          ...
          <div className="form-group">
            <label className="label">出庫日</label>
            <input
              type="date"
              value={movementDate}
